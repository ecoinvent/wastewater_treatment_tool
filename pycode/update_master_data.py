...
def load_MD(MD_pkl_dir, MD_xml_dir, md_fields_xls=None):
    ...
    update_MD_if_needed(MD_pkl_dir, MD_xml_dir, md_fields_xls=None)
    
    if test:
        return None
    else:
        return plk_load(MD_pkl_dir, 'MD')
    
def update_MD_if_needed(MD_pkl_dir, MD_xml_dir, md_fields_xls=None):
    ...
    existing_MD = os.path.join(pkl_dir, 'MD.pkl')
    t_pkl = os.path.getmtime(existing_MD)
    update = t_MD > t_pkl
        if not update:
            print("Master data pkl already up to date"
    else:
...
def build_MD(MD_xml_dir, md_fields_xls, pickle_dump_dir, xls_dump_dir=None):
    ...
    **xls_dump_dir**: path to dir where xls version of master data is dumped. Not used, default=None
    """
    ...
    if xls_dump_dir is not None:
        MD_to_excel(xls_dump_dir, MD)
